            ...
            <div class="bg-gradient-to-r from-purple-600 to-indigo-600 text-white">
                <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                    <div class="flex items-center justify-between">
                        <h1 class="text-3xl font-bold">Record New Attendance</h1>
                ...
                <div class="bg-white shadow-md rounded-lg p-6">
                    <form action="{{ route('admin.attendances.store') }}" method="POST">
                        ...
                        <div class="mb-6">
                            ...
                            <input type="date" id="date" name="date" value="{{ old('date', now()->format('Y-m-d')) }}" required class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            @error('date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                ...
                                <input type="time" id="entry_time" name="entry_time" value="{{ old('entry_time') }}" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                @error('entry_time')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="time" id="exit_time" name="exit_time" value="{{ old('exit_time') }}" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        ...
                        <div class="mb-6">
                            ...
                            <select id="check_in_method" name="check_in_method" required class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                <option value="manual-admin" {{ old('check_in_method') == 'manual-admin' ? 'selected' : '' }}>Manual (Admin)</option>
                                <option value="manual-receptionist" {{ old('check_in_method') == 'manual-receptionist' ? 'selected' : '' }}>Manual (Receptionist)</option>
                                <option value="manual-trainer" {{ old('check_in_method') == 'manual-trainer' ? 'selected' : '' }}>Manual (Trainer)</option>
                                <option value="member-app" {{ old('check_in_method') == 'member-app' ? 'selected' : '' }}>Member App</option>
                                <option value="qr-code" {{ old('check_in_method') == 'qr-code' ? 'selected' : '' }}>QR Code</option>
                                <option value="key-card" {{ old('check_in_method') == 'key-card' ? 'selected' : '' }}>Key Card</option>
                            </select>
