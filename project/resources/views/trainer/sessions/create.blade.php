<!-- resources/views/trainer/sessions/create.blade.php -->
            ...
            <div class="bg-gray-800 shadow-md border-b border-gray-700">
                <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                    <div class="flex items-center justify-between">
                        <h1 class="text-2xl font-semibold text-white">Create New Session</h1>
            ...
            <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                <div class="bg-gray-800 shadow-md overflow-hidden sm:rounded-lg border border-gray-700">
                    <div class="px-4 py-5 sm:p-6">
                        <form action="{{ route('trainer.sessions.store') }}" method="POST">
                            @csrf
                            ...
                            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                ...
                                <div class="sm:col-span-2">
                                    ...
                                    <textarea name="description" id="description" rows="3" 
                                              class="mt-1 focus:ring-orange-500 focus:border-orange-500 block w-full shadow-sm sm:text-sm bg-gray-700 border-gray-600 text-white rounded-md placeholder-gray-400">{{ old('description') }}</textarea>
                                    @error('description')
                                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="city" id="city" value="{{ old('city') }}" required 
                                           class="mt-1 focus:ring-orange-500 focus:border-orange-500 block w-full shadow-sm sm:text-sm bg-gray-700 border-gray-600 text-white rounded-md placeholder-gray-400"
                                           placeholder="e.g. Rabat, Casablanca, Marrakech">
                                    @error('city')
                                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="number" min="1" max="100" name="max_capacity" id="max_capacity" 
                                           value="{{ old('max_capacity', 10) }}" required 
                                           class="mt-1 focus:ring-orange-500 focus:border-orange-500 block w-full shadow-sm sm:text-sm bg-gray-700 border-gray-600 text-white rounded-md">
                                    @error('max_capacity')
                                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                                    @enderror
